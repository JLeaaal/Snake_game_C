...
void anim_death(void) {
    ...
    /* Calcula as informacoes da partida antes de desenhar a tela final */
    int elapsed = (int)(time(NULL) - startTime) - pausedSeconds;
    char buf[100];
    ...
    /* Faz um fade in simples aumentando o destaque da moldura */
    for (step = 0; step < 4; step++) {
        int color;
        if (step == 0) color = 8;
        else if (step == 1) color = 5;
        else if (step == 2) color = 13;
        else color = C_GAMEOVER;

        screen_box(L, T, R, B, color, color);
        screen_clear(L, T, R, B);

        if (step >= 1) {
            screen_center(L, R, msgY,     color, "==============================");
            screen_center(L, R, msgY + 1, color, "      G A M E    O V E R      ");
            screen_center(L, R, msgY + 2, color, "==============================");
            screen_sep(L, T + 6, R, color);
        }

        if (step >= 2) {
            screen_center(L, R, T + 8, 8, "VOCE PERDEU A PARTIDA");
            screen_center(L, R, T + 10, 8, "AS ESTATISTICAS JA ESTAO SENDO MOSTRADAS");
        }

        Sleep(140);
    }

    /* Tela final pronta e persistente, agora com as estatisticas visiveis */
    screen_box(L, T, R, B, C_GAMEOVER, C_PAUSED);
    screen_clear(L, T, R, B);

    screen_center(L, R, msgY,     C_GAMEOVER, "==============================");
    screen_center(L, R, msgY + 1, C_GAMEOVER, "      G A M E    O V E R      ");
    screen_center(L, R, msgY + 2, C_GAMEOVER, "==============================");
    screen_sep(L, T + 6, R, C_GAMEOVER);
    ...
    snprintf(buf, sizeof(buf), "MODO DA PARTIDA    >>   %s",
             autoMode ? "BFS AUTO" : (multiplayerMode ? "2 JOGADORES" : "MANUAL"));
    screen_center(L, R, T + 20, 13, buf);

    snprintf(buf, sizeof(buf), "SKIN USADA         >>   %s", skin_name());
    screen_center(L, R, T + 22, 9, buf);

    screen_sep(L, T + 24, R, C_GAMEOVER);
    ...
    screen_sep(L, B - 4, R, C_GAMEOVER);
    screen_center(L, R, B - 3, 8, "DIGITE SEU NOME E DEPOIS ESCOLHA UMA OPCAO");
}
...
/* Reinicia todas as variaveis e redesenha a partida do zero */
void init_game(void) {
    /* ETAPA 1 - RESET DAS VARIAVEIS GERAIS */
    snakeLen       = 5;
    ...
    /* ETAPA 3 - RESET DO BFS */
    bfs_path_len      = 0;
    bfs_visited_count = 0;

    /* ETAPA 4 - POSICIONAMENTO INICIAL DAS COBRAS */
    int mx = WIDTH / 2;
    int my = (TOP + HEIGHT) / 2;
...
void update_game(void) {
    ...
    /* BLOCO 1 - Decisao de movimento da cobra principal */
    ...
    /* BLOCO 2 - Calcula proxima posicao da cobra 1 */
    int nx1 = snake[0].x;
    int ny1 = snake[0].y;
    if      (dir == DIR_UP)    ny1--;
    ...
    /* BLOCO 3 - Calcula proxima posicao da cobra 2 */
    int nx2 = 0;
    int ny2 = 0;
    if (multiplayerMode) {
        nx2 = snake2[0].x;
    ...
    /* BLOCO 4 - Colisoes com parede e obstaculos */
    if (nx1 < 1 || nx1 > WIDTH || ny1 <= TOP || ny1 >= HEIGHT || is_obstacle(nx1, ny1)) {
        gameOver = 1;
        return;
    }

    if (multiplayerMode && (nx2 < 1 || nx2 > WIDTH || ny2 <= TOP || ny2 >= HEIGHT || is_obstacle(nx2, ny2))) {
        gameOver = 1;
        return;
    }

    /* BLOCO 5 - Colisoes com o corpo */
    int fruitIndex1 = fruit_at(nx1, ny1);
    int fruitIndex2 = multiplayerMode ? fruit_at(nx2, ny2) : -1;

    for (int i = 0; i < snakeLen - (fruitIndex1 == -1 ? 1 : 0); i++) {
    ...
    if (multiplayerMode) {
        for (int i = 0; i < snake2Len - (fruitIndex2 == -1 ? 1 : 0); i++) {
    ...
    /* BLOCO 6 - Move a cobra 1 */
    draw_body(snake[0].x, snake[0].y);
    Segment oldTail1 = snake[snakeLen - 1];
    ...
    /* BLOCO 7 - Move a cobra 2 */
    if (multiplayerMode) {
        draw_body_player2(snake2[0].x, snake2[0].y, 0);
        Segment oldTail2 = snake2[snake2Len - 1];

        for (int i = snake2Len; i > 0; i--) snake2[i] = snake2[i - 1];
        snake2[0].x = nx2;
        snake2[0].y = ny2;
...
void prompt_name_gameover(char *name, int size) {
    ...
    /* Area reservada apenas para a digitacao do nome */
    screen_sep(L, inputY - 2, R, C_GAMEOVER);
    screen_center(L, R, inputY - 1, C_PANEL_HDR, "NOME DO JOGADOR");
    screen_sep(L, inputY, R, C_GAMEOVER);
    ...
    fill_spaces(L + 8, inputY + 1, 45);
    screen_center(L, R, inputY + 1, C_LEVEL, "NOME SALVO COM SUCESSO");
}
...
void input_menu(GameState *state) {
    ...
    else if (k == '1') { multiplayerMode = 0; autoMode = 0; init_game(); menuNeedsRedraw = 1; *state = STATE_PLAYING; }
    else if (k == '2') { multiplayerMode = 0; autoMode = 1; init_game(); menuNeedsRedraw = 1; *state = STATE_PLAYING; }
    else if (k == '3') { multiplayerMode = 1; autoMode = 0; init_game(); menuNeedsRedraw = 1; *state = STATE_PLAYING; }
    else if (k == '4') { rankingNeedsRedraw = 1; *state = STATE_RANKING; }
    else if (k == '5' || k == 27) { *state = STATE_EXIT; }
    else if (k == 13) {
        if      (menuIndex == 0) { multiplayerMode = 0; autoMode = 0; init_game(); menuNeedsRedraw = 1; *state = STATE_PLAYING; }
        else if (menuIndex == 1) { multiplayerMode = 0; autoMode = 1; init_game(); menuNeedsRedraw = 1; *state = STATE_PLAYING; }
        else if (menuIndex == 2) { multiplayerMode = 1; autoMode = 0; init_game(); menuNeedsRedraw = 1; *state = STATE_PLAYING; }
        ...
        if (state == STATE_MENU) {
            ...
            if (!paused) {
                ...
                if (gameOver) {
                    ...
                    /* Mantem a tela de game over aberta e pede o nome nela mesma */
                    prompt_name_gameover(name, sizeof(name));

                    insert_ranking(name, score);

                    /* Depois de salvar o nome, a tela final continua aberta para o usuario escolher */
                    gameOverIndex       = 0;
                    gameoverNeedsRedraw = 1;
